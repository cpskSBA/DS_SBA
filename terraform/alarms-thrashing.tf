resource "aws_cloudwatch_metric_alarm" "thrash" {
  ...
  alarm_name        = "${terraform.workspace}-oppl-dashboard-thrashing"
  alarm_description = <<EOF
The ECS service ${terraform.workspace}-oppl container(s) appear to be thrashing.
...
EOF
  ...
  metric_query {
    ...
    metric {
      ...
      stat        = "Maximum"
      dimensions = {
        ServiceName = "${terraform.workspace}-oppl"
