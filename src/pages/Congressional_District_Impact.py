...
def get_members_API (offset = 0):
    ...
    members = []
    LIMIT = 250
    apikey = st.secrets.Congress['Congress_key']
    ...
    while offset < maxcount:
        ...
        #Carlin's new code
        cols = ["chamber", "endYear", "startYear"]
        df[cols] =df['terms.item'].apply(lambda x:pd.Series([x[0].get(col) for col in cols]))
        ...
        df['House-active']= 0
        df.loc[(df['chamber']== 'House of Representatives') & (df['endYear'] == current_year),'House-active'] = 1

        df['Senate-active']= 0 
        df.loc[(df['chamber']== 'Senate') & (df['endYear']== current_year),'Senate-active'] = 1

        df = df.rename(columns={'district':'member.district','state':'member.state', 'name':'member.name','depiction.imageUrl':'member.depiction.imageUrl','bioguideId':'member.bioguideId'})
    ...
    #replace member.district with district
...
def get_name_affiliation_website (selectrow):
    ...
    key = st.secrets.Congress['Congress_key']
    ...
    #replacing party code with 'partyAbbreviation - partyCode is not longer available
    party = data['member']['partyHistory'][-1]['partyAbbreviation']
