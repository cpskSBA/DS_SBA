...
def table_chart_one(aggregated_df):
    ...
    aggregated_df_chart = aggregated_df_chart.sort_index() #.astype('Int64').fillna(0)
    ...
    avg_row=pd.DataFrame(aggregated_df_chart[cols_to_avg].replace(0, np.NaN).mean(skipna=True)).transpose()
    ...
    "% Orders NOT SET ASIDE": st.column_config.NumberColumn("% Orders NOT SET ASIDE",help="Percent of orders that are NOT A SET-ASIDE under the NAICS code",format="%.2f %%"),
    "% $ NOT SET ASIDE": st.column_config.NumberColumn("% $ NOT SET ASIDE",help="Percent of dollars that are NOT A SET-ASIDE under the NAICS code",format="%.2f %%"),
    ...
    "% Small Business Awards": st.column_config.NumberColumn("% Small Business Awards",help="Percent of orders given to small business under the NAICS Code",format='%.2f %%'),
    ...
    "% Small Business Awarded $": st.column_config.NumberColumn("% Small Business Awarded $",help="Percentage of dollars given to small business under the NAICS Code",format='%.2f %%'),
    ...
    st.dataframe(aggregated_df_chart,column_config = column_config, use_container_width=True,column_order=("Total # Awards","Total Aggregated $","% Orders NOT SET ASIDE","% $ NOT SET ASIDE","# Small Business Awards","% Small Business Awards","Small Business Awarded $","% Small Business Awarded $","Other Than Small Business # Awards","Other Than Small Business Awarded $"))
